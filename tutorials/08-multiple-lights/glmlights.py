...
from tutorials.utils.light import QtLightSource
...
class LightsGL(QOpenGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        # directional light
        self.dirLight = QtShaderLight(
            direction=QVector3D(-0.2, -1.0, -0.3),
            ambient=QtLightSource(intensity=QVector3D(0.1, 0.1, 0.1)),
            diffuse=QtLightSource(
                intensity=QVector3D(192/255, 191/255, 173/255)),
        ...
        self.lamps = [
            QtShaderLight(
                position=QVector3D(0.7, 0.2, 2.0),
                ambient=QtLightSource(intensity=QVector3D(0.1, 0.1, 0.1)),
                diffuse=QtLightSource(
                    intensity=QVector3D(0.7, 0.7, 0.7)),
                specular=QtLightSource(
                    intensity=QVector3D(0.5, 0.5, 0.5)),
                attenuation=QVector3D(1.0, 0.09, 0.032)
            ),
            QtShaderLight(
                position=QVector3D(2.3, -3.3, -4.0),
                ambient=QtLightSource(intensity=QVector3D(0.1, 0.1, 0.1)),
                diffuse=QtLightSource(
                    intensity=QVector3D(1.0, 0.0, 1.0)),
                specular=QtLightSource(
                    intensity=QVector3D(0.5, 0.0, 0.5)),
                attenuation=QVector3D(1.0, 0.09, 0.032)

            ),
            QtShaderLight(
                position=QVector3D(-4.0, 2.1, -5.6),
                ambient=QtLightSource(intensity=QVector3D(0.1, 0.1, 0.1)),
                diffuse=QtLightSource(
                    intensity=QVector3D(1.0, 0.0, 1.0)),
                specular=QtLightSource(
        ...
        self.userLamp = QtShaderLight(
            position=QVector3D(0.0, 1.0, -2.0)
        )
        self.flashLight = QtShaderLight(
            position=self.camera.position,
            direction=self.camera.front,
            ambient=QtLightSource(intensity=QVector3D(0.0, 0.0, 0.0)),
            attenuation=QVector3D(1.0, 0.09, 0.032),
            cutOff=12.5,
            outerCutOff=15.0,
            diffuse=QtLightSource(
        ...
        availableShaders = ["simpleLight", "lamp", "light", "multiLight"]
        self.shaders = {
            name: {
    ...
    def moveLight(self,
        ...
        currentPos = self.userLamp.position
        translationVec = QVector3D(xoffset,
                                   yoffset,
                                   zoffset)
        newpos = currentPos + translationVec
        self.userLamp.setPosition(vec=newpos)
        self.update()
    ...
    def rotateLight(self, xval: float,
        ...
        self.userLamp.setDirection(vec=newRotVec)
        self.update()
    ...
    def changeLampIntensity(self,
        ...
        self.userLamp.setIntensity(colorType=colorType,
                                   channel=channel,
                                   val=val)
        self.update()
    ...
    def changeLampIntensityCoefficient(self,
        ...
        if channel not in availables:
            ...
            raise ValueError(mess)
        #
        self.userLamp.setCoeffs(colorType=colorType,
                                channel=channel,
                                val=val)
        self.update()
    ...
    def changeAmbientLightIntensity(self, xval: float,
                                    yval: float, zval: float):
        ""
        self.userLamp.ambient.setIntensity(
            vec=QVector3D(xval,
                          yval,
                          zval))
        self.update()
    ...
    def changeAmbientLightCoeffs(self, xval: float, yval: float,
                                 zval: float):
        self.userLamp.ambient.setCoeffs(vec=QVector3D(xval, yval, zval))
        self.update()
    ...
    def setLampAttenuation(self,
        ...
        self.userLamp.setAttenuation(vec=vec,
                                     colorType=colorType)
        self.update()

    def setCutOff(self, val: float):
        self.userLamp.setCutOff(val)
        self.update()

    def setOuterCutOff(self, val: float):
        self.userLamp.setOuterCutOff(val)
        self.update()
    ...
    def setDirLightUniforms_proc(self):
        "Set directional light uniform values"
        setvals = {"direction": self.dirLight.direction,
                   "ambient": self.dirLight.ambient.color,
                   "diffuse": self.dirLight.diffuse.color,
                   "specular": self.dirLight.specular.color}
        [
            self.program.setUniformValue("dirLight."+key,
                                         val) for key, val in setvals.items()
        ]

    def setLampsUniform_proc(self):
        "Set uniform values for lamps"
        lsource = QtLightSource()
        def posfn(lsource): return lsource.position
        def attfnc(lsource): return lsource.attenuation.x()
        def attfnl(lsource): return lsource.attenuation.y()
        def attfnz(lsource): return lsource.attenuation.z()
        def ambifn(lsource): return lsource.ambient.color
        def difffn(lsource): return lsource.diffuse.color
        def specfn(lsource): return lsource.specular.color
        setvals = {"position": posfn,
                   "constant": attfnc,
                   "ambient": ambifn,
                   "diffuse": difffn,
                   "specular": specfn,
        ...
        [self.program.setUniformValue(
            name+key,
            val(self.userLamp)) for key, val in setvals.items()
         ]

    def setFlashLightUniforms_proc(self):
        "set flash light values"
        setvals = {"direction": self.flashLight.direction,
                   "position": self.flashLight.position,
                   "constant": self.flashLight.attenuation.x(),
                   "linear": self.flashLight.attenuation.y(),
                   "quadratic": self.flashLight.attenuation.z(),
                   "cutOff": self.flashLight.cutOff,
                   "outerCutOff": self.flashLight.outerCutOff,
                   "ambient": self.flashLight.ambient.color,
                   "diffuse": self.flashLight.diffuse.color,
                   "specular": self.flashLight.specular.color}

        [self.program.setUniformValue("pLight."+key,
                                      val) for key, val in setvals.items()]
    ...
    def setCubeUniforms_proc(self):
        ...
        # end vertex shader
        # fragment shader
        self.setDirLightUniforms_proc()
        self.setLampsUniform_proc()
        self.setFlashLightUniforms_proc()
        self.program.setUniformValue("viewPosition",
    ...
    def setLampUniforms_proc(self):
        ...
        lampModel.translate(self.userLamp.position)
