...
void main(void) {
  ...
  if (theta > light.cutOff) // remember that we're working with angles as
                            // cosines instead of degrees so a '>' is used.
  {
    // ambient
    ...
    // spotlight.
    FragColor =
        vec4(light.ambient * texture(material.diffuse, TexCoords).rgb, 1.0);
  }
}
